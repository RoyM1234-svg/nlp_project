...
def evaluate_model_baseline(
    ...
    for batch in tqdm(data_loader, desc="Generating CoTs"):
    ...
    cot_results_df.to_csv(f"results_{args.model_type}_cot_baseline.csv", index=False)
    print(f"CoT results saved to results_{args.model_type}_cot_baseline.csv")

    del cot_model
    print("CoT model unloaded")
    ...
    final_answer_dataset = DataFrameDataset(cot_results_df)
    ...
    final_results_df.to_csv(f"results_{args.model_type}_baseline.csv", index=False)
    print(f"Final results saved to results_{args.model_type}_baseline.csv")

    predictions_list = [result['predictions'] for result in final_results]
    true_labels_list = [result['true_labels'] for result in final_results]
    
    accuracy = calculate_accuracy(predictions_list, true_labels_list)
